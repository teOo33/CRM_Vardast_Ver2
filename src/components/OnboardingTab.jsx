...
const OnboardingTab = ({ onboardings, openModal, navigateToProfile, setHistoryModalData, onStatusChange }) => {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex justify-between items-center bg-white/80 backdrop-blur p-4 rounded-2xl shadow-sm border border-white shrink-0">
        ...
        <div className="flex gap-2 items-center">
            <div className="flex gap-2 bg-slate-100 p-1 rounded-xl">
                <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg text-xs font-bold flex gap-1 ${viewMode === 'grid' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}><List size={16}/> شبکه</button>
                <button onClick={() => setViewMode('kanban')} className={`p-2 rounded-lg text-xs font-bold flex gap-1 ${viewMode === 'kanban' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}><Columns size={16}/> کانبان</button>
            </div>
              ...
              <div key={item.id} className="bg-white rounded-2xl p-5 shadow-sm border border-indigo-50 hover:shadow-md transition relative overflow-hidden">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center gap-1">
                      {item.history && item.history.length > 0 && <button onClick={() => setHistoryModalData(item.history)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-blue-500"><History size={16}/></button>}
                ...
                {item.created_by && <div className="mt-3 text-[10px] text-gray-400 text-left border-t pt-2">ثبت شده توسط: <span className="font-bold text-gray-500">{item.created_by}</span></div>}
              </div>
            ))}
          </div>
      ) : (
