...
const UserProfile = ({ allUsers, issues, frozen, features, refunds, onboardings, meetings, openModal, profileSearch, setProfileSearch }) => {
    ...
    const handleSearch = (val) => { setSearch(val); setProfileSearch(val); if (val) { const lowerVal = val.toLowerCase(); setSuggestions(allUsers.filter(u => u.username.toLowerCase().includes(lowerVal) || (u.phone_number && u.phone_number.includes(lowerVal))).slice(0, 5)); } else { setSuggestions([]); } };
