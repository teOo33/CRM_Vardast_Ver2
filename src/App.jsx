...
// ✅ new modular imports (مرحله ۲۰ تا ۲۴)
...
// --- Components (همان‌هایی که هنوز داخل App هستند) ---
                  ...
                  <div key={i} className="relative pr-10">
                    ...
                    <div className="bg-slate-50 border rounded-2xl p-4 hover:bg-white hover:shadow-md transition group">
                      ...
                      <div className="font-bold text-sm text-gray-800 mb-2">
                        {r.desc_text || r.reason || r.title || r.conversation_summary || (r.progress ? `پیشرفت: ${r.progress}%` : '')}
                      </div>
      ...
      <main className="flex-1 h-full overflow-y-auto overflow-x-hidden">
        <div className="px-4 sm:px-8 py-6 min-h-full">
          ...
          <Header
            ...
            setGlobalTimeFilter={setGlobalTimeFilter}
            globalCustomRange={globalCustomRange}
            setGlobalCustomRange={setGlobalCustomRange}
            tabTimeFilter={tabTimeFilter}
            setTabTimeFilter={setTabTimeFilter}
            tabCustomRange={tabCustomRange}
            setTabCustomRange={setTabCustomRange}
          />

          {/* Dashboard (همان قبلی، هنوز داخل App) */}
          {activeTab === 'dashboard' && (
          ...
          {/* ✅ AI analysis tab (همان قبلی) */}
          {activeTab === 'ai-analysis' && (
            <AIAnalysisTab issues={filteredIssues} onboardings={filteredOnboardings} features={filteredFeatures} />
          )}

          {/* ✅ Simple frozen/refunds tab (modular) */}
          {['frozen', 'refunds'].includes(activeTab) && (
            <SimpleTableTab
              activeTab={activeTab}
              rows={activeTab === 'frozen' ? filteredFrozen : filteredRefunds}
              openModal={openModal}
              navigateToProfile={navigateToProfile}
            />
          )}
