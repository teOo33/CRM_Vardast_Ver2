...
const useTailwind = () => {
  useEffect(() => {
    if (!document.getElementById('tailwind-cdn')) {
      ...
      // Load script first
      script.onload = () => {
         // Optional: Configure after load if needed, but CDN usually handles defaults
...
const formatDate = (dateStr) => {
  ...
  try {
    ...
    // Check if string before includes
...
const filterDataByTime = (data, range, customRange) => {
    ...
    if (customRange && customRange.length === 2) {
        // customRange is [DateObject, DateObject]
                    ...
                    <tbody className="divide-y dark:divide-slate-700">
                        {filtered.map((m) => (
                            <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-700">
                                ...
                                <td className="p-4">{m.held ? <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-xs font-bold">بله</span> : <span className="bg-gray-100 text-gray-500 px-2 py-0.5 rounded text-xs">خیر</span>}</td>
...
const AIAnalysisTab = ({ issues, onboardings, features }) => {
    ...
    const handleAnalysis = async (type) => {
        setLoading(true);
        let data = [];
        if (type === 'onboarding') { data = onboardings.map(u => ({ progress: u.progress, note: u.meeting_note || u.followup_note })); }
        else if (type === 'features') { data = features.filter(f => f.status !== 'انجام شد').map(f => ({ title: f.title, desc: f.desc_text, status: f.status, votes: f.repeat_count })); }
        else { data = issues.map(i => ({ type: i.type, desc: i.desc_text })); }
...
// ... UserProfile, CohortChart and other components remain roughly same, just updated with safe props ...
const UserProfile = ({ allUsers, issues, frozen, features, refunds, onboardings, meetings, openModal, profileSearch, setProfileSearch }) => {
    ...
    useEffect(() => { const found = allUsers.find(u => u.username === search); setSelectedUserStats(found || null); }, [search, allUsers]);

    const handleSearch = (val) => {
        setSearch(val);
        setProfileSearch(val);
        if (val) { const lowerVal = val.toLowerCase(); setSuggestions(allUsers.filter(u => u.username.toLowerCase().includes(lowerVal) || (u.phone_number && u.phone_number.includes(lowerVal))).slice(0, 5)); } else { setSuggestions([]); }
    };
    ...
    const userRecords = useMemo(() => {
        ...
        ].filter(r => r.username === search).sort((a, b) => (b.date || '').localeCompare(a.date || ''));
...
export default function App() {
  ...
  const [isAuthed, setIsAuthed] = useState(() => { if (typeof window === 'undefined') return false; return localStorage.getItem('vardast_ops_authed') === '1'; });
  ...
  const handleSave = async (e) => { e.preventDefault(); const isEdit = !!editingId; let finalDate = new Date(); if (formData.date) { if (typeof formData.date === 'string') finalDate = parsePersianDate(formData.date) || new Date(formData.date); else if (formData.date?.toDate) finalDate = formData.date.toDate(); else finalDate = formData.date; } const createdTimestamp = finalDate.toISOString(); let table = '', payload = {}; const commonFields = { username: formData.username, phone_number: formData.phone_number, instagram_username: formData.instagram_username, flag: formData.flag || null }; 
    ...
    else if (modalType === 'onboarding') { table = 'onboardings'; payload = { username: formData.username, phone_number: formData.phone_number, instagram_username: formData.instagram_username, telegram_id: formData.telegram_id, has_website: formData.has_website === 'true' || formData.has_website === true, progress: Number(formData.progress), initial_call_status: formData.initial_call_status, conversation_summary: formData.conversation_summary, call_date: formData.call_date, meeting_date: formData.meeting_date, meeting_note: formData.meeting_note, followup_date: formData.followup_date, followup_note: formData.followup_note }; if (!isEdit) payload.created_at = createdTimestamp; if (!isEdit && payload.meeting_date) { const meetingPayload = { username: payload.username, date: payload.meeting_date, meeting_time: '10:00', reason: 'جلسه آنبوردینگ (خودکار)', created_by: currentUser, held: false, history: [] }; supabase.from('meetings').insert([meetingPayload]).then(({ error }) => { if (error) console.error('Error auto-creating meeting:', error); }); } }

    if (!supabase) { const mockId = isEdit ? editingId : Math.floor(Math.random() * 1000000); const finalPayload = { ...payload, id: mockId, created_at: payload.created_at || new Date().toISOString() }; finalPayload.created_by = loggedInUser || 'Unknown'; const updater = (prev) => isEdit ? prev.map(r => r.id === editingId ? { ...r, ...payload } : r) : [finalPayload, ...prev]; if (table === 'issues') setIssues(updater); if (table === 'frozen') setFrozen(updater); if (table === 'features') setFeatures(updater); if (table === 'refunds') setRefunds(updater); if (table === 'profiles') setProfiles(updater); if (table === 'onboardings') setOnboardings(updater); if (table === 'meetings') setMeetings(updater); setIsModalOpen(false); setEditingId(null); setFormData({ ...INITIAL_FORM_DATA }); return; }
    ...
    <div className="h-screen w-screen flex bg-[#F3F4F6] dark:bg-slate-950 overflow-hidden transition-colors duration-300" dir="rtl">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-0 md:w-20'} h-full bg-white/90 dark:bg-slate-900/90 dark:border-slate-800 backdrop-blur-xl border-l border-gray-200 flex flex-col transition-all duration-300 overflow-hidden fixed md:static inset-y-0 right-0 z-50`}>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
             <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition">{darkMode ? <Sun size={18}/> : <Moon size={18}/>}</button>
      ...
      <main className="flex-1 h-full overflow-y-auto overflow-x-hidden">
        <div className="px-4 sm:px-8 py-6 min-h-full">
          ...
          {activeTab === 'profile' && <UserProfile allUsers={allUsers} issues={issues} frozen={frozen} features={features} refunds={refunds} onboardings={onboardings} meetings={meetings} openModal={openModal} profileSearch={profileSearch} setProfileSearch={setProfileSearch} />}
